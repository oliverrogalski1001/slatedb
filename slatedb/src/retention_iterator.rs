...
pub(crate) struct RetentionIterator<T: RowEntryIterator> {
    ...
    /// When set, dropped BlobRef bytes are accumulated per `pack_id` (compaction path).
    /// The compactor uses this to decrement `live_bytes` on each pack.
    dropped_pack_bytes_sink: Option<Arc<Mutex<HashMap<Ulid, u64>>>>,
}
...
impl<T: RowEntryIterator> RetentionIterator<T> {
    ...
    fn record_retention_dropped_pack_bytes(
        sink: &Option<Arc<Mutex<HashMap<Ulid, u64>>>>,
        ordered: &[RowEntry],
        ...
        let mut guard = s.lock().expect("dropped pack bytes sink mutex poisoned");
        for &idx in dropped_indices {
            if let ValueDeletable::BlobRef(br) = &ordered[idx].value {
                *guard.entry(br.pack_id).or_insert(0) += br.length as u64;
            }
        }
    }
    ...
    fn apply_retention_filter(
        ...
        Self::record_retention_dropped_pack_bytes(
            &dropped_pack_bytes_sink,
            &ordered,
            &dropped_indices,
        );
                ...
                RetentionBufferState::NeedProcess => {
                    ...
                    self.buffer.process_retention(|versions| {
                        Self::apply_retention_filter(
                            ...
                            self.dropped_pack_bytes_sink.clone(),
                        )
                    })?;
                }
            }
...
mod tests {
    ...
    fn apply_retention_filter_records_blob_dropped_past_retention_boundary() {
        ...
        versions.insert(
            ...
            RowEntry::new(
                ...
                ValueDeletable::BlobRef(BlobRef::new(pack_id, 0, 8)),
                1,
                None,
                None,
            ),
        ...
        let expected: HashMap<Ulid, u64> = [(pack_id, 8u64)].into_iter().collect();
    ...
    fn apply_retention_filter_records_blob_when_expired() {
        ...
        // Pack bytes recorded for GC.
        let expected: HashMap<Ulid, u64> = [(pack_id, 8u64)].into_iter().collect();
        assert_eq!(*sink.lock().unwrap(), expected);
    }

    #[test]
    fn apply_retention_filter_accumulates_bytes_across_multiple_drops() {
        use crate::test_utils::TestIterator;
        use slatedb_common::clock::MockSystemClock;
        ...
        let sink = Arc::new(Mutex::new(HashMap::new()));
        let mut versions = BTreeMap::new();
        versions.insert(Reverse(5u64), RowEntry::new_value(b"k", b"newest", 5));
        versions.insert(Reverse(4u64), RowEntry::new_value(b"k", b"boundary", 4));
        // Three older versions that all fall outside the retention window.
        versions.insert(
            Reverse(3u64),
            RowEntry::new(
                Bytes::copy_from_slice(b"k"),
                ValueDeletable::BlobRef(BlobRef::new(pack_a, 0, 10)),
                3,
                None,
                None,
            ),
        ...
        versions.insert(
            ...
            RowEntry::new(
                Bytes::copy_from_slice(b"k"),
                ValueDeletable::BlobRef(BlobRef::new(pack_a, 10, 7)),
                2,
                None,
                None,
            ),
        ...
        versions.insert(
            ...
            RowEntry::new(
                Bytes::copy_from_slice(b"k"),
                ValueDeletable::BlobRef(BlobRef::new(pack_b, 0, 5)),
                1,
                None,
                None,
            ),
        ...
        let expected: HashMap<Ulid, u64> = [(pack_a, 17u64), (pack_b, 5u64)].into_iter().collect();
        assert_eq!(*sink.lock().unwrap(), expected);
    }
}
