...
use slatedb::manifest::ManifestCore;
...
async fn wait_for_manifest<F>(db: &Db, mut predicate: F, timeout: Duration) -> ManifestCore
where
    F: FnMut(&ManifestCore) -> bool,
{
    let deadline = Instant::now() + timeout;
    loop {
...
async fn test_blob_write_compact_gc_end_to_end() {
    ...
    //    has drained and B1 shows up in manifest.orphan_blobs.
    let manifest = wait_for_manifest(
        &db,
        |m| {
            m.l0.is_empty()
                && m.last_compacted_l0_sst_view_id.is_some()
                && m.orphan_blobs.iter().any(|o| o.blob_id == b1)
        },
    ...
    assert!(
        orphan.recorded_at_manifest_id > 0,
    ...
    wait_for_manifest(
        &db,
        |m| !m.orphan_blobs.iter().any(|o| o.blob_id == b1),
...
/// in `manifest.orphan_blobs`. The tombstone itself carries no blob, so it
...
async fn test_tombstone_trim_produces_orphan_for_underlying_put() {
    ...
    assert!(
        !manifest.orphan_blobs.iter().any(|o| o.blob_id == b2),
        "B2 is still live (k2 has no tombstone) and must not be an orphan. Got: {:?}",
        manifest.orphan_blobs,
    );
    assert_eq!(
        manifest.orphan_blobs.len(),
        1,
    ...
    //    then run blob GC. B1 gets deleted and pruned from orphan_blobs; B2
